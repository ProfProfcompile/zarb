...
#if defined(NDEBUG)
# error "Zarbit cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6ae31a60de0d28016c6562fd15f1e61683b8efa7707a595c1216c055292ff491");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Zarbit: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Zarbit Signed Message:\n";
...
{
    // Zarbit: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
 int64 nSubsidy = 5 * COIN;
    ...
    // Subsidy is cut in half every 100000 blocks, which will occur approximately every 1 years
    nSubsidy >>= (nHeight / 85000); // Zarbit: 100k blocks in ~1 years
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Zarbit: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Zarbit: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7e85c3e8dc5f23fc1a4fa9dcb6675778393983ccd05878a6e3cf6a60e0c42fe0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Steinhoff in trouble for shareholder investments";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 5 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044fd5814c9614af60858925fc368e387e041f493ee80396e70c327e4d81a7aab737f60df409bb96e1293a1442b9b4854e09629756010c31c6be21f4da57945314") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1540319077;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085823647;

        if (fTestNet)
        {
            block.nTime    = 1540319077;
            block.nNonce   = 386959306;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x637369378d283c0ffaedc6b44a2ddf5d8fafecf75c2464713eaaa3a87a04eb54"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xc3, 0xb1, 0xda }; // Zarbit: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZarbitMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZarbitMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZarbitMiner(CWallet *pwallet)
{
    printf("ZarbitMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zarbit-miner");
    ...
    try { loop {
        ...
        printf("Running ZarbitMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZarbitMiner, pwallet));
}
