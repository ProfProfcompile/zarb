...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Zarbit " + FormatFullVersion();
...
unsigned int pnSeed[] =
{
    0x0100007f,0x916677a0,0x396577a0
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Zarbit is probably already running."), addrBind.ToString().c_str());
        else
